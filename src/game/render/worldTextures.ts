...
/** Rotate a cottage facade 90° CW so east/west walls run as thin side boards. */
export function ensureCottageSideTexture(
  scene: Phaser.Scene,
  sourceKey: string,
): string {
  const sideKey = `${sourceKey}-side`;
  if (scene.textures.exists(sideKey)) {
    return sideKey;
  }
  ...
  canvas.width = frame.cutHeight;
  canvas.height = frame.cutWidth;
  ...
  scene.textures.addCanvas(sideKey, canvas);
  return sideKey;
}
