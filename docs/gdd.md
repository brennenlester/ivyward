...
- **Levels:** creatures level from shared spar XP (actives only). Catalog HP/ATK are Lv 1 baselines; effective combat stats use `floor(base * (1 + (level-1)*(2/49)))` plus shrine bonuses → ~3× at Lv 50. XP to reach level N is `5 * (N - 1)²` (`MAX_LEVEL` = 50; `XP_PER_SPAR_WIN` = 70 shared). No heal on level-up. Wilds scale with per-species spar wins: `min(50, 1 + floor(wins/2) + rarityBias)` where rarityBias is +0 / +3 / +6 from max encounter weight (≥40 / 13–39 / ≤12); sovereigns excluded. Befriend inherits the wild’s effective level.
